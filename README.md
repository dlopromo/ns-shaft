...
- 1P / 2P 本機雙人模式
- Online 2P：4 位全數字房間碼、Co-op lockstep，以及可同步觀看對方動作的 Split Race
- 普通平台、輸送帶、翻滾/消失平台、彈弓、尖刺和頂部壓迫
- 12 格 LIFE、樓層計分、右側難度和 RECORD 顯示
- 原版 WAVE 音效事件映射和 MIDI BGM 播放
- `localStorage` 設定、1P/2P 姓名、排行榜 cache 和失敗提交佇列
- Firebase 全域 Best 5：1P、2P、協力、對戰各自按三種難度排名
...
- Online 2P：選擇 `協力プレイ` 或 `対戦プレイ`，建立房間後會自動複製 4 位數字 code；兩邊按準備完成後倒數 5 秒開始
- `Esc`：暫停 / 繼續
- `F`：全螢幕
...
Firebase Console 必須啟用 **Authentication > Anonymous**。Database 所有資料均放在
`/ns-shaft`，舊 `/rooms` 不再開放；部署本 repo 已測試的規則：
...
兩邊 Ready 後由 host 以 Firebase server time 啟動 `5, 4, 3, 2, 1, GO`。Co-op 會在兩位玩家都死亡後結算；Split Race 先完成的一方可觀看對手，雙方完成後顯示結果 5 秒。兩種模式都會自動回到同一房間、保留設定並清除 Ready，毋須按 Abort 便可再戰；Abort 只用作離開房間。

任何一方可按 Pause。遊戲、輸入、snapshot 和 BGM 會一起停止；雙方都按「再開準備」後，以 server time 倒數 3 秒再開。Split Race 在 5 秒內收不到新 snapshot 時保留最後畫面並顯示通信狀態，恢復連線後自動消失。

房間內的 P1/P2 列會以灰色、黃色和綠色分別顯示等待、已連線和已 Ready。建房時會嘗試把 code 寫入剪貼簿；若瀏覽器拒絕權限，可按 `Copy Code` 重試，或直接複製已自動選取的 code。

Best 5 只顯示 Firebase 全域紀錄。本機不再維護可清除的分數表；正常 Game Over 才提交，Abort、離房和斷線不提交。失敗提交以穩定 UUID 暫存在 `localStorage`，下次連線重試。

如本機已啟動在 `http://127.0.0.1:5175`，可用 `npm run test:firebase`
開兩個隔離 browser context，實測兩種模式的建房、clipboard、入房、Ready、
5 秒倒數、雙方 Pause/3 秒再開、遊戲同步、結果、自動同房再戰及測試房清理。
規則本身可用 `npm run test:firebase-rules` 在 emulator 驗證。
不接觸正式資料的完整 Auth、Rules 與雙 browser context 流程，可用
`npm run test:firebase-browser` 一次驗證。
...
src/
  ...
  game/online/   Firebase 房間、4 位房間碼和 deterministic lockstep
